...
==== Die „Impf-Strategie“: Überleben im System Verbandssport

Was bedeutet das für die Praxis? Dürfen Ihre Athleten nie wieder zu einem traditionellen Kader-Training oder zum Verbandstrainer?

Nun, sicher zum Training mit dem Kader, um gegen gute Sparrings-Partner ihre Fähigkeiten zu erproben.

Aber mit Drill-Training oder zu den persönlichen Lektionen mit Technik-Leitbild? *In einer idealen Welt: Nein*.

In der Realität des deutschen Sportsystems würde das oft das Ende der Karriere bedeuten. Wenn Ihre Athleten dorthin müssen, müssen Sie sie intellektuell bewaffnen. Geben Sie ihnen einen *„mentalen Schutzschild“* mit:

. _Distanzierung_: Erklären Sie ihnen, dass die Drill-Lektion beim Verband eine choreografierte Tanzstunde ist, um den dortigen Trainer zufriedenzustellen. Es ist körperliche Konditionierung, aber kein echtes Fechten.
. _Subversive Variabilität_: Weisen Sie Ihre Athleten an, während des starren Drills _heimlich zu variieren_. Sie sollen den Treffpunkt auf der gegnerischen Weste millimeterweise verschieben, das Timing unsichtbar verzögern oder den Druck der Finger variieren. Nach außen hin sieht es aus wie Gehorsam, aber das Nervensystem bleibt im „Such-Modus“.

Ein Athlet, der weiß, _warum_ der Drill des traditionellen Trainers suboptimal ist, ist dagegen weitgehend immun. Er gibt dem System, was es sehen will, und behält für sich, was er zum Siegen braucht.
...
Als Coach darfst Du Regisseur werden, der das Bühnenbild und das Drehbuch so manipuliert, dass das Drama (der Lerneffekt) maximal wird.
...
// TODO Teile davon ins Coach-Kapitel ziehen
...
** *Stille Post* (No Blade Contact): Treffer zählen nur, wenn keine Klingenberührung stattgefunden hat. (Fördert Disengagement, Tempo und Timing ohne Bindung).
** *Klebstoff-Klinge* (Constant Contact): Treffer zählen nur, wenn vorher eine Klingenbindung bestand. Lösen ist verboten. (Zwingt zu Infight, Kraftübertragung und Fühlen).
** *Der Einbeinige* (Single Leg Takedown): Wer trifft, während er auf einem Bein steht (oder gerade springt), bekommt doppelte Punkte. (Fördert Balance, Koordination und unvorhersehbare Rhythmen).
** *Zeitlupe vs. Hyper-Speed* (Matrix-Mode):
*** _Variante A_: Nur Zeitlupe erlaubt. Wer schneller wird, verliert. (Fördert Präzision und Balance).
*** _Variante B_: Nur maximale Explosivität erlaubt. Kein Abtasten. (Fördert Entscheidungsschnelligkeit).
** *Der 5-Sekunden-Countdown* (Shot Clock): Jeder Angriff muss innerhalb von 5 Sekunden nach dem "Allez" oder nach dem letzten Klingenkontakt erfolgen. Sonst Punktabzug. (Erzwingt Action und verhindert Passivität).
** *Symmetrie-Verbot* (Anti-Mirror): Wenn der Gegner vorwärts geht, musst du vorwärts gehen (Infight erzwingen). Wenn er rückwärts geht, musst du rückwärts gehen (Sog-Wirkung). (Bricht das klassische "Abstand halten"-Muster).
...
** *Blind-Date* (No Vision Start): Start Rücken an Rücken, mit geschlossenen Augen oder eine Abdeckplane zwischen den Fechtern. Auf Signal drehen/Augen auf/Plane entfernen und sofort fechten. (Trainiert Reaktionsschnelligkeit und Orientierung).
...
** *Die Keule*: Fechten mit einem starken Magneten (10g) direkt an der Spitze ("Wedeln" wird fast unmöglich)
** *Der rutschige Griff* (Slippery Handle): Griff mit kleinem Handtuch Tape umwickeln (Zwingt zu lockerem Griff, statt "Krampf-Halten").
** *Das verlängerte Ziel* (Target Extension): Ein Tennisball oder Schwamm wird an der Maske oder am Ellenbogen befestigt. Nur Treffer auf dieses Objekt zählen. (Verändert die Affordance Landscape des Ziels massiv).
** *Der Schildkampf* (Target Restriction): Nutze eine runde Pappe (50cm Durchmesser) als Erweiterung der Glocke (erzwingt ungewöhnliche Stoßwinkel)
** *Die kurze Waffe* (Short Blade): Fechten mit Mini-Degen oder kurzen Schlauchstücken (Zwingt zu naher Distanz und Infight).
** *Der "Falsche" Handschuh* (Sensory Deprivation): Fechten mit einem dicken Fäustling oder einem Boxhandschuh an der Waffenhand (Waffe wird nur "eingeklemmt"). (Nimmt das feine Fingergefühl und zwingt zur Steuerung aus der Schulter/dem Arm).
** *Augenklappe / Brille* (Visual Restriction):
*** Einseitig: Simuliert monokulares Sehen (kein Tiefensehen).
*** Maske Oben/Unten abgeklebt: Man sieht die Beine/Klinge nicht mehr, nur den Kopf/Rumpf. (Zwingt zum Fühlen und Antizipieren).
** *Gummiband-Fesseln* (Resistance Bands): Ein Theraband verbindet beide Knöchel (kurz). (Zwingt zu kleinen, schnellen Schritten und stabiler Basis).
** *Der Rucksack* (Load Carriage): Fechten mit einem (leichten) Rucksack oder einer Gewichtsweste. (Verändert den Schwerpunkt und erhöht die Rumpf-Last).
** *Ball in der Hand* (Dual Task): Die freie Hand muss einen Tennisball jonglieren oder prellen, während man ficht. (Entkoppelt die Körperhälften und bindet kognitive Ressourcen).
** *Das "Laserschwert"* (Poolnudel/Schaumstoff): Statt Degen nutzen wir Poolnudeln. (Nimmt die Angst vor dem Schmerz und erlaubt extrem mutige, aggressive Aktionen ohne Verletzungsrisiko).

* *Zielvorgabe* (Goal / Task Goal)
  Hier definieren wir, was "Erfolg" bedeutet. Beispiele:

** *Sniper-Modus* (Precision only): Treffer zählen nur auf einen 5cm großen Punkt (z.B. Maskenband). Markierung z.B. mit geerdeter Säbelweste (ungültig) und Klebeband darauf (gültig). Alles andere ist "daneben". (Zwingt zu maximaler Präzision).
** *Verkehrte Welt* (Reverse Scoring):
*** Ein Doppeltreffer gibt beiden Minus-Punkte. (Fördert "Einer-Fechten").
*** Ein Treffer auf den Fuß zählt 5 Punkte, auf den Rumpf nur 1. (Lenkt den Fokus extrem auf tiefe Ziele).
** *Der Überlebenskünstler* (Defense First): Fechter A darf nicht angreifen, nur parieren/ausweichen/mitstoßen. Er bekommt einen Punkt, wenn Fechter B nach 30 Sekunden nicht alleine getroffen hat. (Fördert extreme Defensive und Zeitmanagement).
** *Kombinations-Zwang* (Combo-Counter): Ein Punkt zählt erst, wenn zwei Treffer in Folge (innerhalb von 3 Sek.) gesetzt wurden (der erste wird ignoriert/pariert). (Zwingt zum Weiterfechten nach dem ersten Kontakt – Second Intention).
...
*** *Königsmord* (Focus Target): Vor dem Gefecht wird heimlich ein Körperteil ausgelost (z.B. "Linker Oberschenkel"). Nur Treffer dort zählen. Der Gegner weiß es nicht. (Schult das "Lesen" der gegnerischen Absicht).
** *Boden-Kampf* (Ground Control): Ein Punkt wird nur gegeben, wenn der Gegner nach dem Treffer aus dem Gleichgewicht ist (stolpert/wackelt). (Fördert physische Dominanz und "schwere" Treffer).
** *Koop-Treffer* (Synchronicity): (In Partnerübung) Beide müssen gleichzeitig treffen (Doppeltreffer erzwingen). (Schult das Gefühl für das Timing des Gegners).
** *Der "Angsthase"* (Risk Management): Man startet mit 10 Punkten. Jeder eigene Angriff kostet 1 Punkt "Einsatz". Ein Treffer bringt 3 Punkte zurück. (Lehrt ökonomisches Risiko-Management: Lohnt sich der Angriff?).
